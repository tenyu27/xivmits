...
- **Mono for aligned numbers:** timestamps use `ff="monospace"` with `tabular-nums` so a column of `0:23` / `1:14` aligns on the right edge.
...
- **Tank personal-mit rows:** the chosen co-tank pairing’s rows splice into the party timeline right below the party mechanic each names, marked with a `--accent-border` left edge and an `xs` uppercase `--accent-strong` “Personal” tag. An `alts` line (“P1 Double Invuln”) renders as a labelled sub-row of the same action markup. A phase-wide personal note renders as its own edged row at its anchor.
