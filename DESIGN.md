...
- **Tank personal-mit rows:** the chosen co-tank pairing’s rows splice into the party timeline right below the party mechanic each names, marked with a `--accent-border` left edge and an `xs` uppercase `--accent-strong` “Personal” tag. An `alts` line (“P1 Double Invuln”) renders as a labelled sub-row of the same action markup. A phase-wide personal note renders as its own edged row at its anchor. A row may carry a `seat` (`MT` or `OT`) for a job-pair plan whose rows still split by party seat (DMU); it hides for the other seat. An empty-action row is a marker that only names the buster.
...
- **A density toggle.** PiP already *is* the dense mode. A third density to keep in sync buys less than making the two views excellent.
