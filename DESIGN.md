...
- **Buttons:** Mantine `Button` at the default radius. Three variants, no fourth — `filled` (primary, contrast-computed label), `outline` (`Change fight`, the one action that leaves the plan behind), `subtle`+`color="gray"` (recovery action inside an error alert). Set the variant, never the colours. Only one filled button on screen at a time. No outer glow, no custom cursor.
- **Selectors:** real `<select>` (`NativeSelect`) with the label above. The disabled *option text* says why it is locked (“Pick a fight first”), so the reason survives without colour. Selection screen: three selects stack full-width at every breakpoint (matches step order). Focused view: three fixed control rows, one gap between every control, each row wrapping only if it runs out of width. Row 1 (what you are reading): role, job, a “Mits” segmented `All` / `Party` (on `All` the plan picker follows — other-tank for a paired sheet, or P3 boss + P5 invuln for a job-keyed one; on `Party` those are hidden). When the sheet ships `tankMits.priorities`, an info glyph beside the P3 boss / P5 invuln label opens a tooltip listing the suggested job order for each side, the viewer’s own job pulled out in `--accent`. Row 2 (how the page is drawn): display, layout, Notes, and Mechanics. Notes uses Show / Hide and Mechanics uses All / Mit only, matching the other segmented controls. Row 3 (where it is drawn): `Cheatsheet` and `Pop out` — two separate buttons, no label above them, sharing the `default` variant because both open the same plan on another surface; it takes an extra `mt="md"` so the gap above it reads like the label the other rows have. Label above in one style, one shared 36px height, bottom-aligned.
...
- **Mechanics:** a segmented All / Mit only control after Notes in control row 2, All by default and persisted to localStorage. Choosing Mit only removes mechanics with no assignments after job resolution and personal-plan selection. Carry-overs and alternatives count as assignments, and personal continuations keep their mechanic heading. Applies to all layouts, the cheatsheet, and PiP. An empty filtered phase says “No mitigation assigned for this phase.”
